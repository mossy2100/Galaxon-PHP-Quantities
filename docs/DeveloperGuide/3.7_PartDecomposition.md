...
Some quantity types have default part unit symbols and result unit symbols configured in the `QuantityPartsService`. You can read and customise these at runtime:

```php
use Galaxon\Quantities\Services\QuantityPartsService;

// Check the defaults
QuantityPartsService::getPartUnitSymbols('time');   // ['y', 'mo', 'w', 'd', 'h', 'min', 's']
QuantityPartsService::getPartUnitSymbols('angle');  // ['deg', 'arcmin', 'arcsec']

// Customise for your application
QuantityPartsService::setPartUnitSymbols('time', ['h', 'min', 's']);
QuantityPartsService::setResultUnitSymbol('time', 'min');
```

The same configuration is also accessible via the `QuantityType` object's property hooks, which delegate to the service:

```php
Time::getQuantityType()->partUnitSymbols;              // ['h', 'min', 's']
Time::getQuantityType()->partUnitSymbols = ['d', 'h'];  // calls the service
```
