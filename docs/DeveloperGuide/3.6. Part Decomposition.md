...
Each quantity type defines sensible defaults for part decomposition. You can customise these at runtime:

```php
// Check the defaults
Time::getDefaultPartUnitSymbols();      // ['y', 'mo', 'w', 'd', 'h', 'min', 's']
Angle::getDefaultPartUnitSymbols();     // ['deg', 'arcmin', 'arcsec']
