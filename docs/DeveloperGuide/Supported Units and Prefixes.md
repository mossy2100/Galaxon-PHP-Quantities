...
The system uses 10 base dimensions, each with a corresponding SI base unit. Seven of these are the standard SI base units. Three additional "bonus" base dimensions are included to make the system work for angles, data, and currency.
...
\* Bonus dimensions not part of the official ISQ. Angle is treated as dimensionless in SI (a ratio of lengths), but is given its own dimension here for practical unit handling. Currency uses gold troy ounces (XAU) as the base unit, being the least biased and most widely accepted measure of value.

**Note:** The ISQ uses 'Θ' (Greek capital theta) for temperature. 'H' is used here because it's similar in appearance, ASCII characters are easier to type, and 'H' suggests heat.

---

# Prefixes

Units can accept SI metric prefixes, binary prefixes, and sometimes both.
...
- **Metric prefixes**: Standard SI prefixes from quecto (10⁻³⁰) to quetta (10³⁰)
...
All units have an ASCII symbol, so they are easy to type. Therefore, you can use the following:
