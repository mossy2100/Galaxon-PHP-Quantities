...
final class UnitTermTest extends TestCase
{
     ...
     */
    public function testQuantityTypePropertyReturnsQuantityTypeForRegisteredDimension(): void
    {
        $term = new UnitTerm('m');
    ...
    {
        // Create a unit with a dimension that has no registered quantity type.
        $unit = new Unit(
        ...
        try {
            $term = new UnitTerm($unit);
            $this->assertNull($term->quantityType);
        } finally {
    ...
    /**
     * Test tryExpand expands a named compound unit.
     */
    ...
    public function testTryExpandWithExponent(): void
    {
        ...
        $this->assertNotNull($expansion);
        $this->assertSame(1.0, $expansion->value);
        $this->assertSame('kg2*m2/s4', $expansion->compoundUnit->asciiSymbol);
    }
